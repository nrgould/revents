    ...
    .onUpdate(async (snapshot, context) => {
        ...
        if (before.attendees.length < after.attendees.length) {
            let attendeeJoined = after.attendees.filter(
                (item1) =>
                    !before.attendees.some((item2) => item2.id === item1.id)
            )[0];
        ...
        if (before.attendees.length > after.attendees.length) {
            let attendeeLeft = before.attendees.filter(
                (item1) =>
                    !after.attendees.some((item2) => item2.id === item1.id)
            )[0];
    ...
    .onCreate(async (snapshot) => {
        ...
        try {
            const followerDocs = await db
                ...
                .collection('userFollowers')
                .get();
            const user = event.attendees.filter(
            ...
            followerDocs.forEach((doc) => {
                admin
                    .database()
                    .ref(`/posts/${doc.id}`)
                    .push(newPost(user, 'created-event', event.id, event));
            });
        } catch (error) {
            console.log(error);
        }
